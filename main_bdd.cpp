...
void requete(MYSQL *conn)
{
    ...
    if (mysql_query(conn,"SELECT * FROM score"))
    {
...
void fontome::move_fontome(){
    supp_fontome();
     ...
     if(map[boly][bolx] == '_') {setCColor(color[7]); gotoxy(bolx,boly);printf("%c",4);}
...
MYSQL *conn;
    ...
    if(mysql_real_connect(conn,"localhost","mariam","1234","score",0,NULL,0)==NULL)
    {
